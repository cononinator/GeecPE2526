...
* UPDATE: custom delay and test mode enabled for more frequent data aquasition.
*/
...
//Delay customisation variables declaration
int test_mode;
...
void setup() {
  ...
  int test_mode;
  int custom_delay_time;
  test_mode = 1; // 0 means racing, 1 means testing.
  custom_delay_time = 25;  //delay between data aquiring usually 1000ms for racing.
      ...
      if (currentRotationTime >= 100 && currentRotationTime <= 10000 && (currentTime - currentLastPulseTime) < timeout) {
       ...
       // custom delay code for testing
        if (test_mode == 0) {
          ...
          float timeSeconds = currentRotationTime / (custom_delay_time);
        }
  ...
  for (;;) {
    if (I2CactiveTask == NULL) {
      ...
      if(test_mode == 0){
        ...
        vTaskDelay(pdMS_TO_TICKS(custom_delay_time));
      }
